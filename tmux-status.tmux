...
status_right() {
  tmux set -g status-right "#[fg=$color3,bg=$status_bg_color]$separator_start_icon#[fg=$color1,bg=$color3] $time_icon%H:%M:%S #[fg=$color3,bg=$color2]$separator_end_icon#[fg=$color1,bg=$color2] $date_icon%Y/%m/%d #[fg=$color2,bg=$color1]$separator_end_icon $host_icon#H #(tmux-mem-cpu-load --colors --interval 2)!"
